...
PathfinderGUI::PathfinderGUI(std::string windowTitle) {
    ...
    gcAlgorithm->addItem("Dijkstra's algorithm");
...
void PathfinderGUI::loadCurrentlySelectedWorld() {
    ...
    if (startsWith(worldFile, "Random")) {
        ...
        // non-random maze; just load from a file
        loadWorld(worldFile);
    }
}
...
void PathfinderGUI::generateRandomMaze(WorldSize size) {
    if (world) {
        delete world;
        gWindow->repaint();
    }
    ...
    std::cout << std::endl;
    std::cout << "Generating a random maze using Kruskal's algorithm..." << std::endl;
...
bool PathfinderGUI::pathVerify(Vector<Vertex*>& path, Vertex* start, Vertex* end) {\
    BasicGraph* graph = world->getGraph();
...
Vector<Vertex*> PathfinderGUI::runPathSearch() {
    ...
    auto time_elapsed = duration_cast<nanoseconds>(timer_stop - timer_start);
    std::cout << "Time elapsed in executing algorithm : " << time_elapsed.count()
